...
int main(void)
{
	...
	int i=0, iver=0, counterChar=0, readCharTest=0, readCharRef=0;
	int nver=1210;
#ifndef LS_MODE
	...
	time(&startTimer);
	printf("Testing discretizeCube function.\n");

	expectedValue=0;
	PRINT_TEST_START(counter,expectedValue);
	printf("pMesh=%p\n",pMesh);
	returnValue=discretizeCube(pMesh);
	...
	returnValue=discretizeCube(pMesh);
	printf("writingMeshFile returns %d value.\n",writingMeshFile("test0.mesh",pMesh));
	freeMeshMemory(pMesh);
	...
	returnValue=discretizeCube(pMesh);
	printf("writingMeshFile returns %d value.\n",writingMeshFile("test1.mesh",pMesh));
	freeMeshMemory(pMesh);
